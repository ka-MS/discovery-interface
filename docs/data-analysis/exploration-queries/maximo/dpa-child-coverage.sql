...
-- name: nodeid-child-inventory
SELECT O.OBJECTNAME, O.DESCRIPTION
FROM MAXIMO.MAXOBJECT O
WHERE O.OBJECTNAME LIKE 'DPA%'
  AND O.ISVIEW = 0
  AND O.PERSISTENT = 1
  AND EXISTS (
      SELECT 1 FROM MAXIMO.MAXATTRIBUTE A
      WHERE A.OBJECTNAME = O.OBJECTNAME
        AND A.ATTRIBUTENAME = 'NODEID'
  )
