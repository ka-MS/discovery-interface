...
SELECT COALESCE(d.virtualsubtype, d.physicalsubtype, d.type) AS subtype,
       ...
       sum(CASE WHEN EXISTS (SELECT 1 FROM view_ipaddress_v2 i WHERE d.device_pk = ANY(i.device_fks)) THEN 1 ELSE 0 END) AS has_ip,
       sum(CASE WHEN EXISTS (SELECT 1 FROM view_part_v1 p WHERE p.device_fk=d.device_pk) THEN 1 ELSE 0 END) AS has_part,
       sum(CASE WHEN EXISTS (SELECT 1 FROM view_mountpoint_v2 m WHERE d.device_pk = ANY(m.device_fks)) THEN 1 ELSE 0 END) AS has_mount,
