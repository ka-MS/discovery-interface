...
| LASTSCANDT | 최종 발견 시각 | 변환 | `c.last_discovered` | 부모 Computer 값. Part 원천에 없음 |
| HASLD | 상세 설명 있음 | 상수 | – | 0 |
...
분류 `DEV.DISKDRIVE`. 값이 없는 속성은 행을 만들지 않는다.
...
| DISKDRIVE_DISKSIZE | 디스크 크기 | NUMVALUE | 변환 | `pm.hdsize`, `pm.hdsize_unit` | 단위 코드 미결. 5절 |
...
| DISKSIZE 단위 | `hdsize_unit`이 GB·TB 혼재. `TBYTE` 코드 존재 미확인. GB 정규화 대안. ISSUE-11 |
