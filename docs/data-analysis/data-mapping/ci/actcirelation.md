...
2. 재실행 시 관계 한 행 유지와 ACTCIRELATIONID 보존. **확인.** 재실행 후 147건 전 행의 ID가 동일했고, 수동 샘플 6001도 유지됐다.
...
6. 여러 장비 배열·여러 IP를 첫 번째 하나로 줄이지 않는지. **일부 확인.** `filesystem-array-fanout` 조회 결과 0건 — 이번 적재분에 마운트포인트당 복수 장비가 연결된 사례가 있었는지 원천 확인이 필요하다. IP 관계는 아직 구현하지 않았다.
