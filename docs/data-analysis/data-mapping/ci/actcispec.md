...
| CLASSSPECID | ClassSpec ID | BIGINT(19) | Y | 직접 | `c.CLASSSPECID` | 템플릿 ID를 상수로 쓰지 않는다 |
| CLASSSTRUCTUREID | 클래스 구조 | UPPER(25) | N | 직접 | `b.CLASSSTRUCTUREID` | `c.CLASSSTRUCTUREID`와 일치 |
| DISPLAYSEQUENCE | 순서 표시 | SMALLINT(10) | N | 직접 | `u.SEQUENCE` | ACTCI용 설정 |
...
| MANDATORY | 필수적 | YORN(1) | N | 직접 | `u.MANDATORY` | ACTCI용 설정; 필수값 누락은 정상값으로 대체하지 않는다 |
