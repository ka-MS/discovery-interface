...
- 사용자 요청에 따라 설계문서의 영역별 관계도를 보완해 `ci/relations.md`로 옮겼다.
- ComputerSystem·NetworkSystem은 설명용 영역, Host는 물리/VM의 역할임을 명시했다.
- NetworkSystem→IP는 실제 원천 연결과 분류쌍 규칙이 있을 때만 저장됨을 명시했다.
- 설계문서의 중복 그림은 정본 링크로 교체하고 설계 이유·관측 이력은 보존했다.
- 앞선 커밋에서 제외했던 사용자 관계도 변경은 이번 요청으로 정본에 반영한다. 미추적 원천 조사 SQL은 계속 제외한다.
- 검증: 문서 계약 검사(로컬 링크 526개·매핑 앵커·SQL·정의 대응)와 `git diff --check` 통과. 실행 코드·SQL은 변경하지 않았다.
