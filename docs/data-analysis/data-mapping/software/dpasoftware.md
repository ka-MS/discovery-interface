...
| INSTALLPATH | 설치 경로 | ALN(4000) | Y | 직접 | `view_softwareinuse_v1.install_path` | 관측 3/485 · 72/4900, 최대 111자. 자연키 구성 요소지만 원천이 거의 비어 있다 |
...
| SOFTWARENAME | 애플리케이션 | ALN(256) | N | 직접 | `view_software_v1.name` | 관측 485/485 · 4900/4900, 최대 110자. 자연키 구성 요소. DEFAULTVALUE=UNKNOWN |
...
| VERSION | 버전 | ALN(128) | Y | 직접 | `view_softwareinuse_v1.version` | 관측 485/485 · 4894/4900, 최대 56자. 자연키 구성 요소 |
...
- ISSUE-5 — 자연키는 `(NODEID, SOFTWARENAME, VERSION, INSTALLPATH)` 로 확정됐다.
  `.68` 원천 41행이 `(device, name, version)` 기준 중복이고 `install_path` 로
  갈리지 않아 적재 전 중복 제거 규칙이 필요하다.
