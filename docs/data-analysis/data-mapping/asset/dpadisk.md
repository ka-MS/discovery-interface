...
| DISKID | 디스크 ID | BIGINT(19) | N | 채번 | – | 대리키. 원천 `part_pk` 는 재수집 시 바뀌므로 쓰지 않는다 |
...
- `DISKTYPE` 값 대응 규칙 미정. 원천 `media_type_name` 은 관측 전건 `storage` 하나뿐인데 기존 수집분은 `Hard`/`Floppy`/`CD_ROM` 을 쓴다.
- `DISKID` 채번 규칙 미정. DPACPU 의 `CPUID` 와 같은 방식으로 맞춘다.
