...
| 발견 시각·필수값 | LASTSCANDT는 device.last_discovered 사용. 없으면 해당 Computer 적재를 보류·집계하는 안 | 실행 시각을 발견 시각으로 대체하지 않음. LANGCODE·CHANGEBY는 기존 Maximo 설정에 맞는 ETL 설정값, HASLD는 상세 설명 미사용 시 0, CHANGEDATE는 적재 변경 시각으로 제안 |
...
| 저장·검증 단위 | Computer 한 대의 본체와 스펙을 한 트랜잭션으로 반영 | 오류 난 장비는 전체 롤백하고 결과에 실패 표시. 재실행 중복, 값 변경, 빈 값, 분류 변경을 검증 |
