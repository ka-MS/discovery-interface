...
| Host→VM | 기존 Computer CI 사이 관계 | VIRTUALIZES | 원천은 VM의 virtual_host_device_fk가 Host를 참조하며 Host→VM 1:N 규칙·코드 구현 완료. 2026-09-16 실제 관계 연결 검증 완료 |
...
- Host→VM은 신규 VIRTUALIZES를 사용한다. 물리·가상 Host 각각에서 VM으로 향하는 1:N 규칙이며 코드도 같은 방향으로 저장한다.
