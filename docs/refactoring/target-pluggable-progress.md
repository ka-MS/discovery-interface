...
- [x] 문서·아키텍처 검사·전체 test/bootJar·최종 리뷰·로컬 커밋.
...
합의한 구현 범위의 미완료 항목은 없다. 아래 커밋과 검증 명령으로 재확인할 수 있다.
새 타겟 요구가 정해지면 설계 정본의 확장 절차를 따른다. 실제 운영 DB 검증은 별도 승인·환경이 필요한 후속 작업이다.
...
- `dc612a6` — 목표·기준선·설계/기록 위치 확정.
- `84c237d` — 책임별 패키지 분리, 정책/관계 매핑 분리, 조건부 타겟 조립과 회귀 검사.
- `4ad256c` — 중복 원천 조회 공유, 표현 변환 동등성, 독립 테스트 타겟과 검증 도구·매핑 문서.
- 최종 문서 커밋 `docs: 타겟 교체 구조 정본과 최종 검증 기록 확정` — README/CLAUDE/설계 정본/완료 기록.
  이 문서를 포함한 커밋이므로 해시는 `git log -1 --oneline`으로 확인한다.

기준 HEAD 대비 build.gradle·설정 리소스·기존 실행 스크립트·connections.env 및 이전 리팩터링
완료 기록은 변경하지 않았다. 기존 미추적 webwas-ci-source.sql은 커밋에 포함하지 않았다.
중복 Query 네 개와 이전 선택 규칙은 공유 구현으로 대체했으며 기존 내용은 Git 이력으로 복구 가능하다.
이번 작업에서 생성된 Python 캐시만 제거했다. push·merge·PR·DB 작업은 수행하지 않았다.

완료 증거는 기존 전체 테스트 + 추가 의미 동등성/타겟 선택 테스트, 기준 코드 대조,
컴파일 의존 검사, bootJar 검사와 문서 대조의 조합이다. 문자열 SQL 비교만으로
변경된 투영의 의미를 입증하지 않았으며 결과 동등성 테스트를 별도로 실행했다.
