...
public class CiRelationJob implements IntegrationJob {
    ...
    public CiRelationJob(CiRelationQuery query, ActCiRelationWriter writer) {
        this.query = query;
        this.writer = writer;
    }
    ...
    private void integrate(CiRelationSource source) {
        long totalCount = query.getTotalCount(source);
        if (totalCount <= 0) {
        ...
        for (var page : PageLoop.pages(totalCount, DEFAULT_BATCH_SIZE)) {
            ...
            List<ActCiRelationUpsert> data = query.getData(source, offset, limit);
            readCount += data.size();
